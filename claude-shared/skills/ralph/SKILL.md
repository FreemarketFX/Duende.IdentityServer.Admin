...
5. **Re-validate** prd.json against the source PRD (see [Re-validate Against Source PRD](#re-validate-against-source-prd) section below)
6. **Commit** all changes (archive + new files + validation report) before completing
...
## Re-validate Against Source PRD

The converter sometimes drops PRD detail — non-functional requirements, edge cases, secondary AC bullets — that Ralph then never sees during execution. Run a fresh, independent re-validation pass **after** writing prd.json and progress.txt, **before** committing.

### Procedure

1. **Spawn** the `ralph-prd-validator` subagent via the Agent tool. Pass:
   - `PRD_PATH` — absolute path to the source PRD markdown
   - `PRD_JSON_PATH` — absolute path to `tasks/current/prd.json`
2. **Print the report verbatim** to the user.
3. **Always write** the report to `tasks/current/prd-validation.md` — it ships in the commit so Ralph and future reviewers can see what was checked.
4. **If verdict is `clean`** — proceed to commit.
5. **If verdict shows gaps** — ask the user how to proceed (use `AskUserQuestion`):
   - **Apply suggested patches** — edit prd.json to apply the validator's suggested AC bullets / new stories, then **re-spawn the validator once**. Overwrite `prd-validation.md` with the second report. If the second pass is also non-clean, surface it and ask again (Let me edit / Accept as-is) — **do not loop further**.
   - **Let me edit** — pause; user edits prd.json manually, then asks the skill to continue → re-spawn validator once and overwrite `prd-validation.md`, then commit.
   - **Accept as-is** — proceed to commit unchanged. The current `prd-validation.md` documents the accepted gaps.

### `EXTRA` findings always block

Any `EXTRA` (scope-creep) finding makes the verdict non-clean even with zero `MISSING` / `PARTIAL`. Scope creep must never be auto-resolved — the user explicitly accepts it (with that bullet now documented in `prd-validation.md`) or removes it.

### One re-run cap

After auto-patching, the validator runs at most one more time. This bounds cost and avoids infinite "almost clean" loops. If the second pass is non-clean, fall back to the manual / accept-as-is options above.

---

## Final Step: Commit All Changes

After writing prd.json, progress.txt, and the validation report, commit everything in a single commit:
...
- `tasks/current/prd-validation.md` - validator report (clean or accepted gaps)
...
- [ ] **Re-validation pass run** (`ralph-prd-validator`); gaps resolved or explicitly accepted; `prd-validation.md` written
- [ ] **Committed** archive, PRD markdown, prd.json, progress.txt, and prd-validation.md
